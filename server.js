...
io.on("connection", function(socket) {
  ...
  socket.on("login", function(data) {
    ...
    arrUsersOnline.push(data.userId);
    console.log(arrUsersOnline, "arrUsersOnline");
  ...
  socket.on("disconnect", function() {
    ...
    var index = arrUsersOnline.indexOf(users[socket.id]);
    if (index !== -1) {
      arrUsersOnline.splice(index, 1);
    }
    console.log(arrUsersOnline, "arrUsersOnline");
  ...
  socket.on("user-send-request", function(data) {
    ...
    io.sockets.emit("server-send-request", arrUsersOnline);
  });
