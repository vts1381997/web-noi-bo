...
module.exports = {
    ...
    getSearch: (limit, offset, index, sortBy, ns_id_ass, callback) => {
        var query = "select da.dm_duan_ten tenduan, (select fullname from users where name = ht.ns_id_nguoitao) nguoitao, \
        ...
        ht.dm_duan_id left join users us on us.name = '"+ ns_id_ass + "' \
        where  ht.ns_id_ass = (select ns_id from nhansu ns left join users us on ns.ns_dinhdanhcanhan = us.madinhdanh where us.name \
        = '"+ ns_id_ass + "') order by ht.ht_thoigiantiepnhan DESC " + ' offset ' + offset + ' limit ' + limit;
        knex.raw(query)
            .then((res) => {
                knex.raw("select da.dm_duan_ten tenduan, (select fullname from users where name = ht.ns_id_nguoitao) nguoitao, \
                ns.ns_ho || ' ' || ns.ns_tenlot || ' ' || ns.ns_ten nguoiduocgiao, \
                case(ht.ht_uutien) when 'GAP' then 'Gấp' when 'CAO' then 'Cao' \
                when 'TB' then 'Trung bình' when 'THAP' then 'Thấp' when 'RT' \
                then 'Rất thấp' end uutien, to_char(ht.ht_thoigiantiepnhan, \
                ...
                ht.dm_duan_id left join users us on us.name = '"+ ns_id_ass + "' \
                where  ht.ns_id_ass = (select ns_id from nhansu ns left join users us on ns.ns_dinhdanhcanhan = us.madinhdanh where us.name \
                = '"+ ns_id_ass + "') order by ht.ht_thoigiantiepnhan DESC").then((resCount) => {
    ...
    getAlls(data, callback) {
        knex.raw("select fullname from users where name='" + data.username + "'").then((res) => {
            callback({
                data: res.rows
            })
        }).catch((err) => {
    ...
    chat: function (hotros, callback) {
        knex.raw("insert into chat(id, nguoigui, tennguoigui, nguoinhan, tennguoinhan, tinnhan, ngay, gio, url) values ('" + hotros.id + "', '" + hotros.user + "', '" + hotros.name + "', '" + hotros.nguoiNhan + "', (select fullname from users where name='" + hotros.nguoiNhan + "'), '" + hotros.text + "', '" + hotros.ngay + "', '" + hotros.gio + "', '"+ hotros.url +"')").then(response => {
        })
    },
    ...
    getChat: function (data, callback) {
        knex.raw("select * from chat where nguoigui in ('" + data.nguoiGui + "', '"+ data.nguoiNhan +"') and nguoinhan in ('" + data.nguoiGui + "', '"+ data.nguoiNhan +"') order by ngay ASC, gio ASC").then((res) => {
            callback({
                data: {
                    result: res.rows
                }
    ...
    getName: function (cookie, callback) {
        knex.raw("select ns_ho, ns_tenlot, ns_ten, coalesce (ns_ho, '') || ' ' || coalesce (ns_tenlot, '') || ' ' || coalesce (ns_ten, '') as ns_hovaten, ns_id, ns_address from nhansu where ns_dinhdanhcanhan=(select madinhdanh from users where name='" + cookie.user_cookie + "')")
            .then((res) => {
                callback({
                    data: {
                        name: res.rows
    ...
    getHotroById: (ht_id, callback) => {
        knex.raw("select * from (select  hotro.* , nhansu.ns_hovaten, dm_da.dm_duan_ten, khh.kh_ten,khh.kh_sodienthoai, khh.kh_email, nhs.ns_hoten, dvs.dm_dv_ten from hotros hotro left join (select da.dm_duan_ten dm_duan_ten, da.dm_duan_id dm_duan_id from duans da) as dm_da on dm_da.dm_duan_id = hotro.dm_duan_id left join (select kh_ten, kh_sodienthoai, kh_email,kh.kh_id kh_id from khachhangs kh) as khh on khh.kh_id = hotro.kh_id left join (select us.madinhdanh,us.name from users us) as users on users.name = hotro.ns_id_nguoitao left join (select coalesce (ns_ho, '') || ' ' || coalesce (ns_tenlot, '') || ' ' || coalesce (ns_ten, '') as ns_hoten, ns_dinhdanhcanhan from nhansu ns) as nhs on nhs.ns_dinhdanhcanhan=users.madinhdanh left join (select coalesce (ns_ho, '') || ' ' || coalesce (ns_tenlot, '') || ' ' || coalesce (ns_ten, '') as ns_hovaten, ns_id from nhansu ns) as nhansu on nhansu.ns_id=hotro.ns_id_ass left join (select dv.dm_dv_ten, dv.dm_dv_id from donvis dv) dvs on dvs.dm_dv_id=hotro.dm_dv_id) as hotrobyid where hotrobyid.ht_id ='" + ht_id + "' ")
            .then((res) => {
                callback({
                    data: {
                        hotrobyid: res.rows
