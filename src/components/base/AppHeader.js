...
class AppHeader extends Component {
      ...
      if (res) {
        if (res.data.data.name[0] === undefined) {
          ...
          if (res.data.data.name[0].ns_tenlot !== "") {
            tenNguoiDung =
              res.data.data.name[0].ns_ho.trim() +
              " " +
              res.data.data.name[0].ns_tenlot.trim() +
              " " +
              res.data.data.name[0].ns_ten.trim();
          } else {
            tenNguoiDung =
              res.data.data.name[0].ns_ho.trim() +
              " " +
          ...
          localStorage.setItem(
            "urlAvatar",
            res.data.data.name[0].ns_address ||
              "http://103.74.122.80:5000/uploads/unnamed.png"
          );
          this.setState({
            name: tenNguoiDung,
  ...
  insertOrUpdate = async () => {
    ...
    await form.validateFields((err, values) => {
      ...
      if (
        ...
        values.ht_thoigian_dukien_hoanthanh === undefined
      ) {
      ...
      Request(url, "POST", values)
        .then(async (response) => {
          ...
          thongbao.tb_noidung =
            localStorage.getItem("tenNguoiDung") +
            " đã tạo công việc và gán cho bạn vào lúc " +
            thongbao.tb_thoigiantao;
  ...
  onTodoChange = async (value) => {
    ...
    if (value === "daxong") {
      ...
      form.setFieldsValue({
        ht_thoigian_hoanthanh: format(new Date(), "yyyy-mm-dd"),
      });
      form.setFieldsValue({
        ht_thoigian_dukien_hoanthanh: format(new Date(), "yyyy-mm-dd"),
      });
    } else {
      await this.setState({
        date: (
      ...
      <Menu style={{ width: "140px" }}>
        <Menu.Item key="0">
          <a href="/viewprofile">
            {" "}
            <Icon type="user" />{" "}
            <span style={{ marginLeft: "5px" }}> View Profile </span>
          </a>
        </Menu.Item>
        <Menu.Item key="1">
        ...
        <Menu.Item key="2">
          <a onClick={this.logOut} href="/" style={{ color: "red" }}>
            {" "}
            <Icon type="logout" />{" "}
  ...
  callback = (key) => {
    var user_cookie = cookie.load("user");
    if (key == "dadoc") {
      Request("thongbao/getdadoc", "POST", { tb_ns_id: user_cookie }).then(
        (res) => {
          if (res) {
            var noidungdadoc = [];
            res.data.data.thongbaodadoc.map((value, index) => {
              noidungdadoc.push({
                noidungdadoc: value.tb_noidung,
                ht_id: value.tb_ht_id,
                tb_id: value.tb_id,
              });
            });
            this.setState({
  ...
  onClickThongbaoTatcaDadoc = () => {
    Request("thongbao/updatetatcadadoc", "POST", {})
      .then((response) => {
        this.setState({
          countBadge: 0,
        });
  ...
  onClickXoachuadoc = () => {
    Request(`thongbao/delete`, "DELETE", { trangthai: "chuadoc" })
      .then((res) => {
        this.setState({
          countBadge: 0,
        });
      ...
      <Row style={{ width: "540px" }}>
        ...
        >
          <TabPane
            key={"chuadoc"}
            tab={
              <span style={{ fontSize: "14px", fontFamily: "initial" }}>
                Chưa đọc
              </span>
            }
          >
            ...
            >
              ...
              <a
                className="hoverthongbao"
                style={{ marginLeft: "25%", marginRight: "2%" }}
                onClick={this.onClickThongbaoTatcaDadoc}
              >
                {" "}
                Đánh dấu tất cả là đã đọc
              </a>
              <span style={{ marginRight: "2px" }}></span>
              <a className="hoverthongbao" onClick={this.onClickXoachuadoc}>
                {" "}
                Xóa tất cả
              </a>
            </div>
            <div className="demo-infinite-container" style={{ padding: "0px" }}>
              ...
              >
                {this.state.countBadge === 0 ? (
                  ...
                  <Menu onClick={this.onClickThongbao}>
          ...
          >
            ...
            {this.state.noidungdadoc.length === 0 ? (
              <span
                style={{
                  textAlign: "center",
                  display: "block",
  ...
  async componentDidMount() {
    ...
    socket.emit("login", { userId: user_cookie });
  ...
  showModal = async () => {
    ...
    form.setFieldsValue({
      ht_thoigiantiepnhan: format(new Date(), "yyyy-mm-dd"),
    });
    form.setFieldsValue({
      ht_thoigian_dukien_hoanthanh: format(new Date(), "yyyy-mm-dd"),
    });
      ...
      <div>
        <Header style={{ background: "#fff", padding: 0 }}>
          ...
          <Tooltip title="Thông báo">
            ...
            >
              <Badge
                count={this.state.countBadge}
                style={{ fontSize: "12 px", margin: "20px" }}
              >
                <a>
                  <Icon
                    type="bell"
