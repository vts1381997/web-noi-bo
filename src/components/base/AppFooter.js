...
class AppFooter extends Component {
        ...
        for (var i = 0; i < data.length; i++) {
          for (var j = 0; j < userOnline.length; j++) {
            if (data[i].name == userOnline[j]) {
              html =
                html +
                '<div style="display: -webkit-box;"><img id=' +
                data[i].name +
                ' src="http://103.74.122.80:5000/uploads/OK.png"/><p>' +
                data[i].fullname +
                "</p></div>";
            }
          }
          ...
          for (var j = 0; j < userOffline.length; j++) {
            if (data[i].name == userOffline[j]) {
              html1 =
                ...
                '<div style="display: -webkit-box;"><img id=' +
                data[i].name +
                ' src="http://103.74.122.80:5000/uploads/KO.png"/><p>' +
                data[i].fullname +
                "</p></div>";
            }
          }
  ...
  componentDidMount() {
    ...
    socket.on("server-send-request", function(data) {
      userOnline = deduplicate(data);
      localStorage.setItem("listUserOnline", userOnline);
    });
    ...
    return (
      <Footer style={{ textAlign: "center", fontSize: "12px", padding: "5px" }}>
        <div
          ...
          tabindex="0"
          role="button"
          style={{ cursor: "pointer", outline: "none", paddingLeft: "75%" }}
        >
          <Popover
            content={
              <div
          ...
          >
            <svg
              style={{ position: "fixed", bottom: "5%", zIndex: "999" }}
              width="60px"
              height="60px"
              viewBox="0 0 60 60"
            >
